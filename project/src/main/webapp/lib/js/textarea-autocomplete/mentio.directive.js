...
var app = angular
		...
		.directive(
				...
				[
						...
						'$log',
						'$timeout',
						function(mentioUtil, $document, $compile, $log,
								$timeout) {
							return {
								restrict : 'A',
								scope : {
								...
								controller : function($scope, $timeout, $attrs) {
									...
									$scope.bridgeSearch = function(termString) {
										var searchFn = $attrs.mentioSearch ? $scope.search
												: $scope.defaultSearch;
										searchFn({
											term : termString
										});
									...
									$scope.defaultSelect = function(locals) {
										return $scope.defaultTriggerChar
												+ locals.item.label;
									};

									$scope.bridgeSelect = function(itemVar) {
										var selectFn = $attrs.mentioSelect ? $scope.select
												: $scope.defaultSelect;
										return selectFn({
											item : itemVar
										});
									...
									$scope.setTriggerText = function(text) {
										if ($scope.syncTriggerText) {
											$scope.typedTerm = ($scope.trimTerm === undefined || $scope.trimTerm) ? text
													.trim()
													: text;
										}
									};
									...
									$scope.hideAll = function() {
										for ( var key in $scope.triggerCharMap) {
											if ($scope.triggerCharMap
													.hasOwnProperty(key)) {
												$scope.triggerCharMap[key]
														.hideMenu();
											}
										}
									};
									...
									$scope.addMenu = function(menuScope) {
										if (menuScope.parentScope
												&& $scope.triggerCharMap
														.hasOwnProperty(menuScope.triggerChar)) {
											return;
										}
								...
								link : function(scope, element, attrs) {
									...
									if (attrs.mentioItems) {
										...
										var itemsRef = attrs.mentioSearch ? ' mentio-items="items"'
												: ' mentio-items="localItems"';

										scope.defaultTriggerChar = attrs.mentioTriggerChar ? scope
												.$eval(attrs.mentioTriggerChar)
												: '@';
									...
									scope
											.$watch(
													...
													function(newValue) {
														...
														if (scope.triggerCharSet === undefined) {
															$log
																	.error('Error, no mentio-items attribute was provided, '
														...
														var isActive = scope
																.isActive();
														var isContentEditable = scope
																.isContentEditable();

														var mentionInfo = mentioUtil
																.getTriggerInfo(
																		scope
																				.context(),
																		scope.triggerCharSet,
														...
														if (mentionInfo !== undefined
																 ...
																 * editable
																 */
																(isContentEditable && mentionInfo.mentionTriggerChar === scope.currentMentionTriggerChar) || (!isContentEditable && mentionInfo.mentionPosition === scope.currentMentionPosition))))) {
															/**
															 * save selection
															 * info about the
															 * target control
															 ...
															 */
															scope
																	.setTriggerText(mentionInfo.mentionText);
															/*
															 * remember current
															 * position
															 */
															...
															scope
																	.query(
																			mentionInfo.mentionTriggerChar,
															...
															if (macroMatchInfo !== undefined) {
																...
																scope
																		.replaceMacro(
																				macroMatchInfo.macroText,
																				macroMatchInfo.macroHasTrailingSpace);
															} else if (scope.selectNotFound
																	&& currentTypedTerm
																	&& currentTypedTerm !== '') {
		...
		.directive(
				...
				[
						...
						'$window',
						'$document',
						function(mentioUtil, $rootScope, $log, $window,
								$document) {
							return {
								restrict : 'E',
								scope : {
								...
								templateUrl : function(tElement, tAttrs) {
									return tAttrs.mentioTemplateUrl !== undefined ? tAttrs.mentioTemplateUrl
											: 'lib/js/textarea-autocomplete/mentio-menu.tpl.html';
								},
								controller : function($scope) {
									$scope.visible = false;

									// callable both with controller (menuItem)
									// and without controller (local)
									this.activate = $scope.activate = function(
											item) {
									...
									// callable both with controller (menuItem)
									// and without controller (local)
									this.isActive = $scope.isActive = function(
											item) {
									...
									// callable both with controller (menuItem)
									// and without controller (local)
									this.selectItem = $scope.selectItem = function(
											item) {
										...
										if (typeof text.then === 'function') {
											/*
											 * text is a promise, at least our
											 * best guess
											 */
											text
													.then($scope.parentMentio.replaceText);
										} else {
											$scope.parentMentio
													.replaceText(text);
										}
									...
									$scope.activateNextItem = function() {
										var index = $scope.items
												.indexOf($scope.activeItem);
										this.activate($scope.items[(index + 1)
												% $scope.items.length]);
									};
									...
									$scope.activatePreviousItem = function() {
										var index = $scope.items
												.indexOf($scope.activeItem);
										this
												.activate($scope.items[index === 0 ? $scope.items.length - 1
														: index - 1]);
									};
								...
								link : function(scope, element) {
									...
									if (scope.parentScope) {
										...
										if (!scope.forElem) {
											$log
													.error('mentio-menu requires a target element in tbe mentio-for attribute');
											return;
										}
										if (!scope.triggerChar) {
											$log
													.error('mentio-menu requires a trigger char');
											return;
										}
										// send own scope to mentio directive so
										// that the menu
										// becomes attached
													...
													function(items) {
														if (items
															...
															scope
																	.activate(items[0]);
															if (!scope.visible
																	&& scope.requestVisiblePendingSearch) {
																scope.visible = true;
									...
									scope.$watch('isVisible()', function(
											visible) {
										// wait for the watch notification to
										// show the menu
										if (visible) {
											var triggerCharSet = [];
											triggerCharSet
											...
											mentioUtil.popUnderMention(
													scope.parentMentio
															.context(),
													triggerCharSet, element,
													scope.requireLeadingSpace);
										}
									});

									scope.parentMentio.$on('$destroy',
											function() {
												element.remove();
											});
									...
									scope.adjustScroll = function(direction) {
										...
										var menuItemsList = menuEl
												.querySelector('ul');
										var menuItem = (menuEl
												.querySelector('[mentio-menu-item].active') || menuEl
												.querySelector('[data-mentio-menu-item].active'));
			...
			return function(val) {
				...
				function() {
					...
					return function(matchItem, query, hightlightClass) {
						if (query) {
							var replaceText = hightlightClass ? '<span class="'
									+ hightlightClass + '">$&</span>'
									: '<strong>$&</strong>';
							return ('' + matchItem).replace(new RegExp(
									escapeRegexp(query), 'gi'), replaceText);
						} else {
							return matchItem;
						}
					};
