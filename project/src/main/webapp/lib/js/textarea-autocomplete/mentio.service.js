		...
		.factory(
				...
				function($window, $location, $anchorScroll, $timeout) {
					...
					function popUnderMention(ctx, triggerCharSet, selectionEl,
							requireLeadingSpace) {
						var coordinates;
						var mentionInfo = getTriggerInfo(ctx, triggerCharSet,
						...
						if (mentionInfo !== undefined) {
							...
							if (selectedElementIsTextAreaOrInput(ctx)) {
								coordinates = getTextAreaOrInputUnderlinePosition(
										ctx, getDocument(ctx).activeElement,
										mentionInfo.mentionPosition);
							} else {
								coordinates = getContentEditableCaretPosition(
										ctx, mentionInfo.mentionPosition);
							}
					...
					function scrollIntoView(ctx, elem) {
						// cheap hack in px - need to check styles relative to
						// the element
						var reasonableBuffer = 20;
						var maxScrollDisplacement = 100;
						var clientRect;
						var e = elem[0];
						while (clientRect === undefined
								|| clientRect.height === 0) {
						...
						if (elemTop < 0) {
							$window.scrollTo(0, $window.pageYOffset
									+ clientRect.top - reasonableBuffer);
						} else if (elemBottom > $window.innerHeight) {
							var maxY = $window.pageYOffset + clientRect.top
									- reasonableBuffer;
					...
					function selectedElementIsTextAreaOrInput(ctx) {
						...
						if (element !== null) {
							...
							return (nodeName === 'INPUT' && type === 'text')
									|| nodeName === 'TEXTAREA';
						}
						return false;
					}
					...
					// public
					function replaceMacroText(ctx, targetElement, path, offset,
							macros, text) {
						...
						if (macroMatchInfo.macroHasTrailingSpace) {
							macroMatchInfo.macroText = macroMatchInfo.macroText
									+ '\xA0';
							text = text + '\xA0';
						}
						...
						if (macroMatchInfo !== undefined) {
							...
							if (selectedElementIsTextAreaOrInput(ctx)) {
								...
								var endPos = macroMatchInfo.macroPosition
										+ macroMatchInfo.macroText.length;
					...
					// public
					function replaceTriggerText(ctx, targetElement, path,
							offset, triggerCharSet, text, requireLeadingSpace,
							hasTrailingSpace) {
						resetSelection(ctx, targetElement, path, offset);

						var mentionInfo = getTriggerInfo(ctx, triggerCharSet,
								requireLeadingSpace, true, hasTrailingSpace);
						...
						if (mentionInfo !== undefined) {
							if (selectedElementIsTextAreaOrInput()) {
								...
								pasteHtml(
										...
										mentionInfo.mentionPosition,
										mentionInfo.mentionPosition
												+ mentionInfo.mentionText.length
												+ 1);
							}
						}
					}
					...
					function getMacroMatch(ctx, macros) {
						...
						if (effectiveRange !== undefined
							...
							if (effectiveRange.length > 0
									&& (effectiveRange
											.charAt(effectiveRange.length - 1) === '\xA0' || effectiveRange
											.charAt(effectiveRange.length - 1) === ' ')) {
								hasTrailingSpace = true;
								// strip space
								effectiveRange = effectiveRange.substring(0,
							...
							angular
									.forEach(
											macros,
											function(macro, c) {
												var idx = effectiveRange
														.toUpperCase()
														.lastIndexOf(
																c.toUpperCase());

												if (idx >= 0
														&& c.length + idx === effectiveRange.length) {
													var prevCharPos = idx - 1;
													if (idx === 0
															|| effectiveRange
																	.charAt(prevCharPos) === '\xA0'
															|| effectiveRange
																	.charAt(prevCharPos) === ' ') {
					...
					// public
					function getTriggerInfo(ctx, triggerCharSet,
							requireLeadingSpace, menuAlreadyActive,
							hasTrailingSpace) {
						...
						if (effectiveRange !== undefined
							...
							if (mostRecentTriggerCharPos >= 0
								...
								triggerChar = effectiveRange.substring(
										mostRecentTriggerCharPos,
										mostRecentTriggerCharPos + 1);
								var firstSnippetChar = currentTriggerSnippet
										.substring(0, 1);
								var leadingSpace = currentTriggerSnippet.length > 0
										&& (firstSnippetChar === ' ' || firstSnippetChar === '\xA0');
								if (hasTrailingSpace) {
									currentTriggerSnippet = currentTriggerSnippet
											.trim();
								}
								if (!leadingSpace
										&& (menuAlreadyActive || !(/[\xA0\s]/g
												.test(currentTriggerSnippet)))) {
									return {
					...
					function getTextPrecedingCurrentSelection(ctx) {
						...
						if (selectedElementIsTextAreaOrInput(ctx)) {
							...
							if (selectedElem != null) {
								...
								var selectStartOffset = getWindowSelection(ctx)
					...
					function getContentEditableCaretPosition(ctx,
						...
						markerEl.appendChild(getDocument(ctx).createTextNode(
								markerTextChar));
					...
					function getTextAreaOrInputUnderlinePosition(ctx, element,
							position) {
						var properties = [ 'direction', 'boxSizing', 'width',
								'height', 'overflowX', 'overflowY',
								'borderTopWidth', 'borderRightWidth',
								'borderBottomWidth', 'borderLeftWidth',
								'paddingTop', 'paddingRight', 'paddingBottom',
								'paddingLeft', 'fontStyle', 'fontVariant',
								'fontWeight', 'fontStretch', 'fontSize',
